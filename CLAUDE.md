...
2. On `connection`, `initializeUser` joins the socket to a room named after `user.user_id`, marks the user `connected` in Redis, broadcasts presence to **all** friends, and emits `FRIENDS_LIST` **as `{ friends, hasMore, cursor }`** (the first page from `getConversationsPage` — the inbox, **sorted by latest message** — presence-enriched) + `MESSAGES` (the recent N per conversation for **only that first page's** conversations, from Postgres). Both loads are **bounded** — not the whole friend list / whole chat history. A "friend" on the wire is a backward-compatible superset: `{ user_id, username, full_name, connected, conversationId, lastMessage }`.
