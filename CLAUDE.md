...
- **PostgreSQL** (via **Drizzle ORM** + node-postgres `pg` pool, `db/index.ts`) holds the `users`, `fcm_tokens`, `friendships`, `conversations`, `conversation_members`, and `messages` tables. Schema in `db/schema/*` (one file per bounded context: `users.ts`, `friendships.ts`, `fcmTokens.ts`, `conversations.ts`, `conversationMembers.ts`, `messages.ts` — **no barrel**; each context file is imported directly by the repository that owns it, keeping cross-context dependencies explicit and each slice extractable); typed data-access in `db/repositories/*` (e.g. `users.ts`, `fcmTokens.ts`, `friendships.ts`, `conversations.ts`, `messages.ts`). The Drizzle client (`db/index.ts`) is table-agnostic (no aggregate `schema`); each query supplies its own table. Tables reference each other by the stable `user_id` value with **no cross-context FKs** (per roadmap principle 2). `users.full_name` is the display name shown in the list/header (username stays the identity + presence key). `friendships` stores one **canonical row** per pair (`user_a_id < user_b_id`, enforced by a CHECK) and is the _authorization_ context (`areFriends` gates messaging); `getFriends` joins `users` for usernames.
...
2. On `connection`, `initializeUser` joins the socket to a room named after `user.user_id`, marks the user `connected` in Redis, broadcasts presence to **all** friends, and emits `FRIENDS_LIST` **as `{ friends, hasMore, cursor }`** (the first page from `getConversationsPage` — the inbox, **sorted by latest message** — presence-enriched) + `MESSAGES` (the recent N per conversation for **only that first page's** conversations, from Postgres). Both loads are **bounded** — not the whole friend list / whole chat history. A "friend" on the wire is a backward-compatible superset: `{ user_id, username, full_name, connected, conversationId, lastMessage, unreadCount }`.
...
5. **Read state:** opening a conversation emits `MARK_READ` (`{ conversationId, messageId }`), which advances that member row's `last_read_message_id`. The client also clears its own badge optimistically and increments it locally on a `DIRECT_MESSAGE` for a non-active conversation (`bumpConversation(..., { incrementUnread })`) — a local echo only; the authoritative count is re-derived server-side on the next load.
...
Messaging is **conversation-centric**: a message belongs to a `conversations` row, **not** a user pair (a 1:1 chat is a 2-member conversation; a group is an N-member one — the shape that makes groups a later feature, not a later migration). The old Redis dot-joined `"messageId.to.from.content"` lists are long gone, so `content` round-trips losslessly (dots included).

**The inbox / sort model (the load-bearing decision — read before touching it).** `conversation_members` is the per-user conversation-state row and carries `last_message_id`, a **denormalized sort key** (a pointer into `messages.id`, which is `bigserial` and therefore monotonic). The sidebar read is a single index range-scan over `conversation_members (user_id, last_message_id DESC NULLS LAST, created_at DESC)` — flat as total conversations grow — with `created_at` (member-since) as the fallback order for conversations that have no messages yet. This is **fan-out on write**: `services/sendMessage.ts` does the message `INSERT` **and** one bounded `UPDATE conversation_members … WHERE conversation_id` in a **single transaction**, so the list can never show a stale "latest message". The fan-out is bounded by _this conversation's_ member count (never system-wide) and is deliberately **synchronous — there is no queue**; a broker belongs only to (a) huge-group fan-out and (b) the inter-service event bus at the microservices split. Delivered receipts slot onto the same member row later as a `last_delivered_message_id` pointer — no new infrastructure. Do not "optimize" this into a messages-table join or a query-time LATERAL: the sort key must stay co-located with the membership filter, or the read stops being O(page).

**Unread is a pointer, never a counter.** `conversation_members.last_read_message_id` is the source of truth; the count is **derived** in `getConversationsPage` by a correlated subquery (`messages` newer than the pointer, not sent by the viewer) served by `messages_conversation_id_id_idx`. This is a deliberate rejection of a denormalized `unread_count` column: `GREATEST(COALESCE(last_read_message_id,0), $id)` (see `markConversationRead`) is **idempotent and commutative**, so it converges under concurrent devices _and_ under a broker's at-least-once redelivery, whereas `unread_count + 1` corrupts permanently and undetectably on a single replay — the exact failure mode that appears when fan-out moves behind a queue. It also keeps unread on the **read side** of the future service boundary (recomputable from `messages`) rather than making the send path mutate inbox state. `MARK_READ` (`utils/socket/markRead.ts`) needs no ownership lookup: the UPDATE is scoped to the caller's own member row, so naming someone else's conversation updates zero rows.

⚠️ The unread subquery is **not** the "query-time LATERAL" the paragraph above forbids — that rule protects the _sort key_. This lives in the `SELECT` list, runs only on rows the keyset already chose, and executes at most `limit + 1` times. Verified by `EXPLAIN ANALYZE` against a 2 000-row no-message tail: `Presorted Key: last_message_id, created_at` intact, 17 index rows scanned, subquery `loops=16`.

⚠️ **The `ORDER BY` must spell `NULLS LAST` on _both_ sort keys, matching the index exactly.** `created_at` is `NOT NULL`, so `DESC` and `DESC NULLS LAST` are semantically identical — but the planner matches index ordering **literally**, and a plain `created_at DESC` (i.e. NULLS FIRST) silently drops it to `Presorted Key: last_message_id` alone. That is invisible in the messaged section (`last_message_id` is unique ⇒ sort groups of 1) and catastrophic in the no-message tail, where every row shares `last_message_id = NULL` — one giant sort group — making the read O(all my conversations). Measured: a 2k-no-message inbox scanned 2000 rows instead of 17. If you touch this ORDER BY or the index, re-check the plan with `EXPLAIN ANALYZE` and confirm `Presorted Key: last_message_id, created_at` and an index-scan row count of ~page size.

Friend add/remove create and tear down the conversation **atomically with the friendship** (`handleSocketAddFriend` / `handleRemoveFriend`, each one transaction) — a friendship without a conversation is invisible, since the sidebar is built from `conversation_members`. Any test/dev helper that seeds a friendship must create the conversation too.

On the wire a message is `{ id, messageId, conversationId, to, from, content, createdAt }` — `id` (bigserial) is the pagination cursor, the client dedupes on `messageId`. `to`/`from` are retained for the client (which still keys messages by the conversation partner): `from` is `sender_user_id`, and `to` is the non-sender participant, supplied by the send path and computed by reads via a CASE join on the conversation pair. Repositories: `db/repositories/conversations.ts` (`getOrCreateDirectConversation`, `getConversationsPage` — the inbox query, `bumpConversationLastMessage`, `deleteConversationCascade`) and `db/repositories/messages.ts` (`saveMessage`, `getConversation`, `getRecentMessagesForConversations`), both keyed by `conversation_id`; the row→wire mapper `toWireMessage(row, to)` and page-size constants live in `utils/socket/common.ts`. On the client, a new message (live receive in `ChatMessages`, or the send ack in `ChatBox`) floats its conversation to the top via `utils/bumpConversation.js`; history/initial loads must **not** reorder.
...
- **FCM: `FIREBASE_SERVICE_ACCOUNT_JSON`** (raw JSON or base64) is the **only** source of Firebase credentials — there is no on-disk fallback, so no image layer, repo, or working tree ever holds the credential; every environment injects it from a secret store. Prefer base64: the service account's private key is multi-line, which `.env` files and most secret UIs mangle. `DISABLE_FCM=true` stubs FCM out entirely (used by Compose, CI and E2E); with FCM enabled and the var unset, boot fails fast with an actionable message.
...
- **Write queries with the Drizzle builder first.** Drop to a raw `sql` ``template only for constructs Drizzle does not define — currently `LEAST`/`GREATEST`, `CASE WHEN`, `COALESCE`, row-constructor comparisons `(a,b) < (c,d)`, `DESC NULLS LAST` in `orderBy`, and partial-index predicates — and say which builder was missing in a comment. This is not only style: raw templates lose schema awareness. `alias(messages, "unread")` interpolated into a raw``sql` ` renders as the bare alias (`FROM "unread"` → `relation does not exist`), while the same `alias()` **through the builder** correctly emits `FROM "messages" "unread"`. Build correlated subqueries with the builder and embed via ``sql`(${qb.getSQL()})` `` (see `unreadCount` in `db/repositories/conversations.ts`). After any such rewrite, re-check `EXPLAIN ANALYZE` — a semantically identical rewrite can still change the plan.
