...
const valid = { username: USERNAME, password: PLAINTEXT };
...
describe("registerUser", () => {
    ...
    it("rejects credentials the login form would reject, without touching the DB", async () => {
        ...
        const tooShort = await registerUser({ username: "demo", password: PLAINTEXT });
    ...
    it("rejects a short password", async () => {
        const result = await registerUser({ username: USERNAME, password: "pw" });
    ...
    it("rejects a username with disallowed characters", async () => {
        const result = await registerUser({ username: "bad name!", password: PLAINTEXT });
