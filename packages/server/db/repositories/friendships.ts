...
// `db` or a transaction handle — friend-add/remove run inside a transaction together with the
// conversation lifecycle (create/delete), so the friendship and its conversation stay consistent.
...
// NOTE: the friends-list *page* now lives in the messaging context as `getConversationsPage`
// (db/repositories/conversations.ts) — sorted by latest message, served from conversation_members.
// The old friendship-recency `getFriendsPage` was retired when messages moved to conversations.
